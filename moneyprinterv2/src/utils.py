...
def rem_temp_files() -> None:
    ...
    Keeps `*.json` (account cache) and `*.mp4` (rendered Shorts), so returning to the
    YouTube menu does not delete the last video — PNG/WAV/SRT scratch files are removed.
    ...
    for file in files:
        lower = file.lower()
        if lower.endswith(".json") or lower.endswith(".mp4"):
            continue
        try:
...
def choose_random_song_optional() -> Optional[str]:
    """
    Returns a random audio file path from Songs/, or None if folder missing/empty.
    """
...
def choose_random_song() -> str:
    ...
    """
    try:
        path = choose_random_song_optional()
        if path is None:
