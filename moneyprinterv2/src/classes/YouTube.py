...
# Pillow 10+ removed Image.ANTIALIAS; MoviePy 1.x still uses it in resize.fx
try:
...
class YouTube:
    ...
    def __init__(
        ...
        lock_file = os.path.join(self._fp_profile_path, "parent.lock")
        if os.path.isfile(lock_file):
            raise ValueError(
                "Профиль Firefox занят: закрой Firefox полностью (macOS: Cmd+Q), "
                "подожди несколько секунд и запусти снова. Нельзя использовать один профиль "
                "в двух окнах — пока открыт обычный Firefox, Selenium не стартует."
            )

        self.options.add_argument("-profile")
        self.options.add_argument(self._fp_profile_path)

        # Явный путь к Firefox на macOS (иначе иногда «Process unexpectedly closed with status 0»)
        if sys.platform == "darwin":
            mac_ff = "/Applications/Firefox.app/Contents/MacOS/firefox"
            if os.path.isfile(mac_ff):
                self.options.binary_location = mac_ff
    ...
    @staticmethod
    def _coerce_image_prompt_list(parsed) -> List[str]:
        """Turn JSON (list or dict with imagePrompts / image_prompts, etc.) into list[str]."""
    ...
    def generate_prompts(self) -> List[str]:
        ...
        """
        # Cap count: long scripts + naive len/3 explodes API calls and confuses small LLMs
        raw_n = max(3, int(len(self.script or "") / 80))
        n_prompts = min(raw_n, 12)
        ...
        ["detailed scene 1", "detailed scene 2", "detailed scene 3"]

        You may alternatively use this exact shape:
        {{"image_prompts": ["...", "..."]}}

        Each string must be a concrete visual description (setting, lighting, mood) for image generation.
        Do not repeat the same marketing slogan in every item. No meta-commentary about "viral" or the prompt itself.

        For context, here is the video script:
        {self.script}
        """
        ...
        try:
            ...
            image_prompts = self._coerce_image_prompt_list(parsed)
            ...
            m = r.search(completion)
            if m:
                try:
                    image_prompts = self._coerce_image_prompt_list(
    ...
    def generate_image_nanobanana2(self, prompt: str) -> str:
        ...
        for attempt in range(max_retries):
            ...
            if response.status_code == 429:
                wait_s = min(120, 5 * (2 ** min(attempt, 5)))
                warning(
                    f"Gemini rate limit (429). Waiting {wait_s}s before retry "
            ...
            try:
                ...
                if get_verbose():
                    warning(f"Nano Banana 2 invalid JSON: {str(e)}")
                return None
        ...
        warning("Nano Banana 2: exhausted retries after repeated rate limits.")
        return None
    ...
    def combine(self) -> str:
        ...
        if not self.images:
            error(
                "No images were generated (Gemini returned errors or rate limits). "
                "Wait a few minutes and run again, or check quota at Google AI Studio."
            )
            raise RuntimeError("combine: no images")
        ...
        random_song = choose_random_song_optional()
        ...
        if random_song:
            ...
            warning(
                "No music in Songs/ — using voice-only audio. "
                "Add .mp3/.wav files to moneyprinterv2/Songs/ or set zip_url in config.json."
            )
    ...
    def generate_video(self, tts_instance: TTS) -> str:
        ...
        # Generate the Images (stagger requests to reduce Gemini 429 bursts)
