...
/// Логотип ODO Business Hub
/// Использует изображение из assets, если доступно, иначе кастомный виджет
class OdoBusinessHubLogo extends StatelessWidget {
  final double size;
...
class _CustomBusinessHubLogo extends StatelessWidget {
  final double size;
  final Color? blueColor;
  final Color? greenColor;

  const _CustomBusinessHubLogo({
    required this.size,
    this.blueColor,
    this.greenColor,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      height: size,
      child: CustomPaint(
        painter: _BusinessHubLogoPainter(blue: blue, green: green),
      ),
    );
  }
}

class _BusinessHubLogoPainter extends CustomPainter {
  final Color blue;
  final Color green;

  _BusinessHubLogoPainter({
    required this.blue,
    required this.green,
  });
