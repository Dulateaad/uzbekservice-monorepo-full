# Замена API сервера на защищенную версию
...
# 1. Создаем резервную копию старого сервера
cp vps_api_server.js vps_api_server.js.backup

# 2. Получаем API ключ из .env (для проверки)
API_KEY=$(grep API_KEY .env | cut -d'=' -f2)
echo "API ключ из .env: $API_KEY"

# 3. Создаем защищенный сервер
...
# Проверяем логи (должно быть сообщение о защите API)
...
# Health check (должен работать без ключа)
...
1. Получите API ключ:
   ```bash
   grep API_KEY /var/www/api/.env | cut -d'=' -f2
   ```
...
4. Замените `YOUR_API_KEY_HERE` на реальный ключ из .env
