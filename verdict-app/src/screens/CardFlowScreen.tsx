...
export function CardFlowScreen({ subsection, mode: _mode, onBack }: CardFlowScreenProps) {
      ...
      try {
        ...
        if (firestoreCards.length > 0) {
          ...
          const filtered = subsection && subsection !== 'champion'
            ...
            : fallback;
          setCards(filtered.length ? filtered : fallback);
        }
        setIndex(0);
      } catch {
        if (cancelled) return;
  ...
  const handleChoice = useCallback((choice: 'A' | 'B') => {
    ...
    voteCard(currentCard.id, choice).catch(() => {});
  ...
  }, [currentCard, animating, cards.length]);
  ...
  return (
    <div className="min-h-full bg-[var(--tg-theme-bg-color)] text-[var(--tg-theme-text-color)] flex flex-col pb-[calc(env(safe-area-inset-bottom)+80px)]">
      {/* Кнопка назад — к выбору подраздела */}
      <div className="flex-shrink-0 px-4 py-2 flex items-center gap-2">
        <button
          ...
          className="flex items-center gap-1 text-[var(--tg-theme-hint-color)] active:opacity-70 text-sm"
        >
          <span>←</span>
          <span>Назад</span>
        </button>
      ...
      <div className="flex-1 flex flex-col min-h-0 relative overflow-hidden">
        <div className="flex-1 grid grid-cols-1 grid-rows-2 gap-1 p-2">
          {/* Option A — фоном показываем imageA, если есть */}
          <button
            onClick={() => handleChoice('A')}
            disabled={animating}
            className={`relative rounded-2xl overflow-hidden bg-[var(--tg-theme-secondary-bg-color)] flex items-center justify-center min-h-[120px] transition-all duration-300 ${
        ...
        {/* Проценты под карточками (61% vs 39%) — после голосования */}
        {lastChoice && (
        ...
        {showStreak && !animating && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full bg-[var(--tg-theme-button-color)] text-[var(--tg-theme-button-text-color)] font-bold animate-pulse">
            🔥 Серия: {streak}
          </div>
        )}
      </div>

      {/* Bottom bar: Stop, Share, Stats */}
      <div className="fixed bottom-0 left-0 right-0 flex justify-around items-center py-3 px-4 bg-[var(--tg-theme-bg-color)] border-t border-[var(--tg-theme-hint-color)]/20 pb-[calc(env(safe-area-inset-bottom)+12px)]">
        <button
          onClick={isPaused ? handleResume : handleStop}
          className="flex flex-col items-center gap-1 text-[var(--tg-theme-hint-color)] active:opacity-70"
        >
          <span className="text-xl">{isPaused ? '▶️' : '⏸'}</span>
          <span className="text-xs">{isPaused ? 'Продолжить' : 'Стоп'}</span>
        </button>
        <button
          onClick={handleShare}
          className="flex flex-col items-center gap-1 text-[var(--tg-theme-link-color)] active:opacity-70"
        >
      ...
      {showStats && (
        ...
        >
          <div
            className="bg-[var(--tg-theme-secondary-bg-color)] rounded-2xl p-6 max-w-sm w-full"
          ...
          >
            ...
            <button
              onClick={() => setShowStats(false)}
              className="mt-4 w-full py-2 rounded-xl bg-[var(--tg-theme-button-color)] text-[var(--tg-theme-button-text-color)] font-medium"
            >
              Закрыть
            </button>
          </div>
      ...
      {isPaused && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-10">
          <div className="bg-[var(--tg-theme-secondary-bg-color)] rounded-2xl p-6 text-center">
            <p className="mb-4">Поток остановлен</p>
            <button
              onClick={handleResume}
              className="px-6 py-2 rounded-xl bg-[var(--tg-theme-button-color)] text-[var(--tg-theme-button-text-color)] font-medium"
            >
              Продолжить
            </button>
          </div>
