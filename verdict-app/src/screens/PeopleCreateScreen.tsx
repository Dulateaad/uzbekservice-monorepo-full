...
export function PeopleCreateScreen({ onCreated, onBack }: PeopleCreateScreenProps) {
  const { t } = useLocale();
  const { user } = useUser();
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const authorId = getTelegramUserId() ?? user?.userId ?? undefined;
      await createCard(a, b, 'people', authorId);
  ...
  return (
    <div className="bg-[var(--app-bg)] text-[var(--app-text)] min-h-full p-4 pb-[env(safe-area-inset-bottom)]">
      ...
      <h2 className="text-xl font-bold mb-1">{t.createPeopleCard}</h2>
      <p className="text-[var(--app-text-muted)] text-sm mb-6">{t.peopleCardDesc}</p>
