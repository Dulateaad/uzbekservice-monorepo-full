/**
 * Fix truck_id column type and populate with UUID references
 */
...
async function fixAndSeedData() {
  ...
  try {
    console.log('\n Fixing truck_id column type and seeding data...\n');
    ...
    if (checkResult.rows.length > 0) {
      ...
      console.log(\Current truck_id type: \\);
    ...
    if (trucks.length === 0) {
      console.log(' No trucks found. Skipping seeding.');
      await pool.end();
      return;
    }
    ...
    const ordersResult = await client.query('SELECT id FROM orders WHERE truck_id IS NULL');
    const orders = ordersResult.rows;

    console.log(\Found \ orders without truck_id\);
    ...
    for (const order of orders) {
      ...
      await client.query(
        'UPDATE orders SET truck_id = \, city = \ WHERE id = \',
        [randomTruck, randomCity, order.id],
      );
      updated++;
    }

    console.log(\ Updated \ orders with truck_id and city\);
