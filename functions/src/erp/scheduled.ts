 ...
 */
export const erpAnalyticsDailyRollup = functions
  .runWith({timeoutSeconds: 300, memory: '512MB'})
  .pubsub.schedule('every day 03:00')
  .timeZone('UTC')
  .onRun(async () => {
    const {start, end, dateStr} = utcYesterdayBounds();
    const companiesSnap = await db.collection(ERP_COLLECTIONS.companies).get();
    ...
    for (const cdoc of companiesSnap.docs) {
      ...
      const plSnap = await db
        .collection(ERP_COLLECTIONS.plEntries)
      ...
      const cashSnap = await db
        .collection(ERP_COLLECTIONS.cashMovements)
        .where('companyId', '==', companyId)
        .where('date', '>=', start)
        .where('date', '<', end)
        .get();
      ...
      for (const d of cashSnap.docs) {
        ...
        if (typ === 'in') cashNet += amt;
        else if (typ === 'out') cashNet -= amt;
      }

      const docId = `${companyId}_${dateStr}`;
      await db
        .collection(ERP_COLLECTIONS.analyticsDaily)
        .doc(docId)
        .set({
          companyId,
          date: dateStr,
