...
exports.anamaMergePilotSession = void 0;
const functions = require("firebase-functions");
const admin = require("firebase-admin");
/**
 * RMSSD по RR-интервалам в мс (пилот, не клиника).
 */
function computeRmssd(rrMs) {
    if (rrMs.length < 2)
        return 0;
...
function stressStatusFromRmssd(rmssd) {
    if (rmssd <= 0 || Number.isNaN(rmssd))
        return 'unknown';
    if (rmssd >= 35)
        return 'green';
    if (rmssd <= 15)
...
function pickInsightId(ageBand, stress, quiz) {
    const self = String(quiz.stress_self || '');
    if (ageBand === 'parent_0_5') {
        if (stress === 'red' || self === 'high')
            return 'stress_resilience_parent';
        if (stress === 'green')
            return 'motor_leadership_parent';
        return 'negotiation_empathy_parent';
    }
    if (stress === 'red' || self === 'high')
        return 'stress_resilience';
    if (ageBand === 'young_adult')
        return 'motor_leadership';
    return 'negotiation_empathy';
}
const INSIGHT_TITLE_RU = {
...
function buildRecommendationRu(stress, quiz) {
    const mood = String(quiz.mood || '');
    const sleep = String(quiz.sleep || '');
    return (`Сводка пилота (не диагноз): вариабельность — ориентир «${stress}». ` +
...
async function tryGeminiRecommendationRu(opts) {
    ...
    try {
        ...
        const prompt = 'Ты помощник образовательного пилота эмоционального самочувствия (не врач, не диагноз).\n' +
        ...
        const text = (_b = (_a = r.response) === null || _a === void 0 ? void 0 : _a.text()) === null || _b === void 0 ? void 0 : _b.trim();
...
function buildDailyPatch(stressStatus, ageBand, quiz) {
    const daily = {
...
exports.anamaMergePilotSession = functions.region('us-central1').https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Нужна анонимная авторизация Firebase.');
    }
    ...
    try {
        ...
        if (histSnap.exists()) {
            ...
            if (Array.isArray(h)) {
                rrList = h.filter((x) => typeof x === 'number');
            }
            else if (h && typeof h === 'object') {
                rrList = Object.values(h).filter((x) => typeof x === 'number');
            }
        }
        if (rrList.length < 2) {
            const teleSnap = await db.ref(`users/${deviceId}/telemetry`).once('value');
            const tele = teleSnap.val();
    ...
    await fs.collection('anama_pilot_daily').doc(dayKey).set(buildDailyPatch(stressStatus, ageBand, quiz), { merge: true });
    return {
        rmssd,
        stressStatus,
        insightId,
...
//# sourceMappingURL=anama_pilot.js.map