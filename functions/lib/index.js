...
const CLICK_MERCHANT_ID = ((_a = functions.config().click) === null || _a === void 0 ? void 0 : _a.merchant_id) || process.env.CLICK_MERCHANT_ID || '';
const CLICK_SERVICE_ID = ((_b = functions.config().click) === null || _b === void 0 ? void 0 : _b.service_id) || process.env.CLICK_SERVICE_ID || '';
const CLICK_SECRET_KEY = ((_c = functions.config().click) === null || _c === void 0 ? void 0 : _c.secret_key) || process.env.CLICK_SECRET_KEY || '';
// Конфигурация OneID (получить из Firebase Config или environment variables)
const ONEID_CLIENT_ID = ((_d = functions.config().oneid) === null || _d === void 0 ? void 0 : _d.client_id) || process.env.ONEID_CLIENT_ID || 'odo_uz';
const ONEID_CLIENT_SECRET = ((_e = functions.config().oneid) === null || _e === void 0 ? void 0 : _e.client_secret) || process.env.ONEID_CLIENT_SECRET || '8H8dcZ118ix2arY7w5ObjrfN';
// Используем HTTP redirect_uri вместо custom scheme (OneID требует HTTP/HTTPS)
const ONEID_REDIRECT_URI = ((_f = functions.config().oneid) === null || _f === void 0 ? void 0 : _f.redirect_uri) || process.env.ONEID_REDIRECT_URI || 'https://us-central1-odo-uz-app.cloudfunctions.net/oneidCallback';
...
const TELEGRAM_BOT_TOKEN = ((_g = functions.config().telegram) === null || _g === void 0 ? void 0 : _g.bot_token) || process.env.TELEGRAM_BOT_TOKEN || '';
const TELEGRAM_CHAT_ID = ((_h = functions.config().telegram) === null || _h === void 0 ? void 0 : _h.chat_id) || process.env.TELEGRAM_CHAT_ID || '';
// Конфигурация Email (получить из Firebase Config или environment variables)
const EMAIL_HOST = ((_j = functions.config().email) === null || _j === void 0 ? void 0 : _j.host) || process.env.EMAIL_HOST || 'smtp.gmail.com';
const EMAIL_PORT = parseInt(((_k = functions.config().email) === null || _k === void 0 ? void 0 : _k.port) || process.env.EMAIL_PORT || '587');
const EMAIL_USER = ((_l = functions.config().email) === null || _l === void 0 ? void 0 : _l.user) || process.env.EMAIL_USER || '';
const EMAIL_PASSWORD = ((_m = functions.config().email) === null || _m === void 0 ? void 0 : _m.password) || process.env.EMAIL_PASSWORD || '';
const EMAIL_FROM = ((_o = functions.config().email) === null || _o === void 0 ? void 0 : _o.from) || process.env.EMAIL_FROM || EMAIL_USER || 'noreply@anama.app';
const EMAIL_FROM_NAME = ((_p = functions.config().email) === null || _p === void 0 ? void 0 : _p.from_name) || process.env.EMAIL_FROM_NAME || 'Anama App';
/**
 * Генерация подписи для Click API
 */
function generateClickSignature(clickTransId, serviceId, secretKey, merchantTransId, amount, action, signTime) {
 ...
 */
exports.clickPrepare = functions.https.onRequest(async (req, res) => {
    // CORS headers
    ...
    try {
        const { orderId, amount, userId, cardType } = req.body;
 ...
 */
exports.clickComplete = functions.https.onRequest(async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
    res.set('Access-Control-Allow-Headers', 'Content-Type');
    if (req.method === 'OPTIONS') {
    ...
    try {
        const { click_trans_id, merchant_trans_id, amount, action, sign_time, sign_string, error, error_note, } = req.body;
        // Верифицируем подпись
        const expectedSign = generateClickSignature(click_trans_id, CLICK_SERVICE_ID, CLICK_SECRET_KEY, merchant_trans_id, amount, action, sign_time);
 ...
 */
exports.clickWebhook = functions.https.onRequest(async (req, res) => {
    try {
        const { click_trans_id, merchant_trans_id, amount, action, sign_time, sign_string, error, error_note, } = req.body;
        // Верифицируем подпись
        const expectedSign = generateClickSignature(click_trans_id, CLICK_SERVICE_ID, CLICK_SECRET_KEY, merchant_trans_id, amount, action, sign_time);
 ...
 */
exports.oneidHealth = functions.https.onRequest(async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    res.json({
        status: 'ok',
        service: 'oneid',
 ...
 *   - state: CSRF токен
 */
exports.oneidLogin = functions.https.onRequest(async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    try {
        const { redirect_uri, state } = req.query;
        const finalRedirectUri = redirect_uri || ONEID_REDIRECT_URI;
        const finalState = state || generateOneIdState();
 ...
 */
exports.oneidCallback = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        const error = req.method === 'GET' ? req.query.error : (_b = req.body) === null || _b === void 0 ? void 0 : _b.error;
        ...
        let tokenResponse;
        try {
            tokenResponse = await axios_1.default.post(ONEID_TOKEN_URL, tokenParams.toString(), {
                headers: {
            ...
            console.error('❌ Error exchanging code for token:', ((_c = error.response) === null || _c === void 0 ? void 0 : _c.data) || error.message);
            res.status(500).json({
        ...
        let userInfo;
        try {
            const userInfoResponse = await axios_1.default.get(ONEID_USER_INFO_URL, {
                headers: {
            ...
            console.error('❌ Error fetching user info:', ((_e = error.response) === null || _e === void 0 ? void 0 : _e.data) || error.message);
        ...
        console.log('✅ User authenticated:', ((_f = response.user) === null || _f === void 0 ? void 0 : _f.full_name) || ((_g = response.user) === null || _g === void 0 ? void 0 : _g.sub));
 ...
 *   - Authorization: Bearer {access_token}
 */
exports.oneidUser = functions.https.onRequest(async (req, res) => {
    var _a, _b;
    res.set('Access-Control-Allow-Origin', '*');
    ...
    try {
        ...
        try {
            ...
            console.error('❌ Error fetching user info:', ((_a = error.response) === null || _a === void 0 ? void 0 : _a.data) || error.message);
            res.status(500).json({
...
exports.sendTelegramMessage = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        const telegramResponse = await axios_1.default.post(telegramApiUrl, {
 ...
 */
exports.sendParentalConsentOtp = functions.https.onRequest(async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
    res.set('Access-Control-Allow-Headers', 'Content-Type');
    if (req.method === 'OPTIONS') {
    ...
    try {
        const { email, otp, language = 'ru' } = req.body;
        if (!email || !otp) {
